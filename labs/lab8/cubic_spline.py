...
class CubicSplineInterpolator:
    ...
    def evaluate(self, evaluation_points: np.ndarray) -> np.ndarray:
        ...
        for i in range(self.num_intervals - 1):
            ...
            indices = np.where((evaluation_points >= x_left) & (evaluation_points <= x_right))[0]
            for idx in indices:
                spline_values[idx] = self._evaluate_spline_segment(
                    x_left, self.function_values[i], x_right, self.function_values[i + 1],
                    self.second_derivatives[i], self.second_derivatives[i + 1], evaluation_points[idx]
                )
    ...
    def _compute_second_derivatives(self) -> np.ndarray:
        ...
        for i in range(1, num_nodes - 3):
            coefficient_matrix[i, i - 1], coefficient_matrix[i, i], coefficient_matrix[i, i + 1] = 1 / 12, 1 / 3, 1 / 12
        
        coefficient_matrix[num_nodes - 3, num_nodes - 4], coefficient_matrix[num_nodes - 3, num_nodes - 3] = 1 / 12, 1 / 3
        
        rhs_vector = np.array([
            (self.function_values[i + 2] - 2 * self.function_values[i + 1] + self.function_values[i]) /
            (2 * self.step_size**2) for i in range(num_nodes - 2)
        ])
        second_derivatives = np.insert(inv(coefficient_matrix) @ rhs_vector, [0, len(rhs_vector)], 0)
    ...
    def _evaluate_spline_segment(
        ...
        linear_term_left = (f_left / step_size) - (step_size * second_derivative_left / 6)
        linear_term_right = (f_right / step_size) - (step_size * second_derivative_right / 6)
        return (
